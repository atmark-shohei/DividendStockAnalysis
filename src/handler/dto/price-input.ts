/**
 * 株価入力欄のパース。**handler 層の入力検証**。
 *
 * 仕様: `docs/02_design/logic/dividend-yield-scoring.md` §3.1
 * 「数値のみ入力可能。初期値は空」「全角数字は半角に正規化する」
 *
 * ドメインではなく handler に置くのは、これが「外から来た文字列を信用しない」
 * 処理だからである。ドメインは検証済みの数値だけを受け取る。
 */
...
export function parsePriceInput(raw: string): PriceInputResult {
  const normalized = raw
    ...
    // 全角スペース（U+3000）。正規表現に直接書くと no-irregular-whitespace に当たるため
    // コードポイントで指定する
    .replace(/　/g, ' ')
    // U+2212 MINUS SIGN。IME が半角ハイフンの代わりに出す
    .replace(/−/g, '-')
    .trim();
