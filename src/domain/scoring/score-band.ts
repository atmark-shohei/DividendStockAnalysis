...
/**
 * 素の数値で区分表を引く。比較を整数式に組み替える必要がない指標（%・倍・年）向け。
 *
 * `NaN` を渡すと `lookupPoints` の肯定形判定により `null` に倒れる。
 * 呼び出し側で `NaN` を弾いていても、ここで二重に守る。
 */
export function lookupPointsByValue(bands: readonly ScoreBand[], value: number): number | null {
  return lookupPoints(bands, (threshold) => value - threshold);
}
 ...
 *
 * **ユーザーが閾値を上書きできるようになったら、テストではなく実行時に必ず通すこと。**
 * 穴のある表は `lookupPoints` が `null` を返すため、画面には理由なく `—` が出る。
 */
export function validateBands(
  ...
  if (top === undefined) return err({ kind: 'ThresholdEmpty' });
  if (top.maxExclusive !== null) {
    return err({ kind: 'ThresholdTopBounded', maxExclusive: top.maxExclusive });
  }
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    ...
    if (lower.maxExclusive !== upper.minInclusive) {
      return err({
        ...
        lowerPoints: lower.points,
        lowerMax: lower.maxExclusive,
        upperPoints: upper.points,
        upperMin: upper.minInclusive,
      });
    }
  }
...
export function assertContiguous(bands: readonly ScoreBand[]): void {
  ...
  switch (error.kind) {
    case 'ThresholdEmpty':
      throw new Error('区分表が空です');
    case 'ThresholdTopBounded':
      throw new Error(
        `区分表の最上位に上限があります（maxExclusive=${String(error.maxExclusive)}）。` +
          '最上位は「下限以上」で開いている必要があります',
      );
    case 'ThresholdNotAscending':
      throw new Error(
        `区分表に穴または重複があります: ${error.lowerPoints}点の上限 ${String(error.lowerMax)} と ` +
          `${error.upperPoints}点の下限 ${String(error.upperMin)} が一致しません`,
      );
    default:
      throw new Error(`区分表が不正です: ${error.kind}`);
  }
}
