...
> ステータス: 🟢 F-50（検索・ソート・ページング）は FE・BE とも実装済み（2026-08-19、T-094）。
> 🔴 F-51（解析ダイアログのモーダル化）は未着手（T-096。行クリックは既存のインライン表示のまま）
...
- **2026-08-19**（T-094）: `frontend/pages/ListPage.tsx` を改修し実装。`q`/`sort`/`page`
  （検索・ソート・ページング）、空状態（q空/絞り込み0件の2分岐）、ロード中の `<Skeleton>`、
  総合点の `<ScoreBar>`（`--font-size-score`。§0で「`--font-size-hero` は使わない」と確定）を
  反映。**§4.1 の stretched button パターン・行クリックでのダイアログ化（F-51）は
  未実装のまま**（Manager確認済み。既存の「表示」「削除」ボタン方式のインライン表示を維持）。
  §5 の空状態には、q が空（登録が1件も無い）ケースに限り admin にだけ「銘柄登録」への
  誘導ボタンを追加した（設計書に記載の無い仕様。Manager決定に基づく実装）
...
- ソート `<select>` の表示ラベル（`created_desc`/`score_desc`/`score_asc`/`code_asc` の順）:
  「登録が新しい順」「総合点が高い順」「総合点が低い順」「コード順」
  （設計書に文言指定が無かったため実装時に決定。Manager承認済み、T-094）
...
- 総合点の表示は `{totalScore} / {maxTotalScore} 点`（分母 `maxTotalScore` も併記する）。
  §3 のレイアウト例の「62点」は簡略表記であり、実装は分母を省略しない
  （`scoring-requirements.md` §0.5 の「有効指標数の併記」と同じ思想で、
  総合点も「100点満点中の62点」であることを一覧単体で読み取れるようにするため。
  Manager承認済み、T-094）
...
- admin 向け誘導ボタンの文言は「銘柄登録へ」（`/input` へ遷移）。
  設計書に文言指定が無かったため実装時に決定。Manager承認済み、T-094
