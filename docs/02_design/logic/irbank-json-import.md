...
> 📌 **2026-08-10 追記。⑤ もこの経路の外で埋まる見込み。**
> [edinet-history-import.md §2.8](./edinet-history-import.md) で、有価証券報告書から
> **純利益と自己資本を取って ROE を自算**でき、その値が上表の IRバンク公表 ROE と
> **0.01pp 以内で一致**することを実データで確認した（3銘柄12年度）。
> 上表で ⑤ が ❌ の 7203・1301（実績4期）は、その経路が入れば埋まる。
> **④⑦ も同経路で埋まる見込みに変わっている**（同書 §3）。
> なお **EDINET の公表 ROE 列は期首期末平均基準で、この表の ROE 列（期末基準）とは
> 別物**なので、そのまま混ぜてはいけない（同書 §2.8）。
