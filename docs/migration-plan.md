...
| 6   | **⑧ 金融業の扱い**                       | 🟡 保留                                                      | 現状 `null` → 総合点で 10点分不利。金融株を扱うときに決める。**EDINET 経由でも埋まらない**（[edinet-history-import.md §2.9.3](./02_design/logic/edinet-history-import.md)。未実測）          |
| 6b  | **⑧ 営業利益率の入力（金融業以外）**     | 🔴 未着手（2026-08-11 設計済み・T-054〜T-056）               | 営業利益はハイライト表に無く損益計算書本表にある（1書類2期）。有報を1本追加取得して5期にする。[§2.9・§4.7](./02_design/logic/edinet-history-import.md)。⑧ のための DB 変更は不要             |
| 6c  | **EDINET パース結果のキャッシュ**        | 🔴 未着手（2026-08-12 **決定済み**・T-057）                  | 案b 採用。`docId` キーで D1 へ保存し、**無効化を持たない**（`docId` は不変）。再登録時の取得が 3本 → 2本。[§4.8・§7.8](./02_design/logic/edinet-history-import.md)。**T-055 より先に入れる** |
