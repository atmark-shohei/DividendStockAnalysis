...
| #   | 項目                                    | 状態      | 根拠・次の一手                                                                        |
| :-- | :-------------------------------------- | :-------- | :------------------------------------------------------------------------------------ |
| 1   | **TSV / CSV の取り込み（F-01〜F-04）**  | 🔴 未着手 | 現在は年度別の数値をフォームに直接入力する。Phase 2（T-021〜）                        |
| 2   | **`Sen` の branding を集約まで通す**    | 🟡 部分   | [ADR-0004](./adr/0004-sen-branding-scope.md)。取り込み層の実装後                      |
| 3   | **`UserScoringPolicy`（閾値の上書き）** | 🔴 未導入 | [ADR-0005](./adr/0005-thresholds-fixed-for-now.md)。T-003/T-004 待ち                  |
