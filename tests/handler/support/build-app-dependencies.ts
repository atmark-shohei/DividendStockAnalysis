...
/** `findById(TEST_ADMIN_USER.id)` にだけ応答する。それ以外は「想定外」として落とす */
export function fakeUserRepository(): UserRepository {
  return {
    findByEmail: () => unimplemented('UserRepository.findByEmail'),
    findById: (id: number): Promise<User | null> =>
      Promise.resolve(id === TEST_ADMIN_USER.id ? TEST_ADMIN_USER : null),
...
/** `findById(TEST_ADMIN_SESSION_ID)` にだけ、遠い未来に失効する有効なセッションを返す */
...
/**
 * `AppDependencies` の認証系フィールドをまとめて返す。
